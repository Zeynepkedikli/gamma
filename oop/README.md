📂 Project Descriptions
🔢 Algorithm & Number Programs

Biggest_Number_Program
Finds the largest number among given inputs using basic comparison logic.

Factorial_numbers
Calculates the factorial of a given number using iterative/recursive methods.

Factorial_Numbers_Test
Test cases for validating factorial calculations.

Fibonacci_Numbers
Generates Fibonacci sequences based on user input.

Fibonacci__Numbers_Test
Tests for Fibonacci number generation.

Prime_Numbers
Displays prime numbers within a given range.

Prime_Numbers_2
Alternative implementation of prime number logic.

Prime_Display_Test
Tests for prime number display functionality.

Prime Counter Test
Counts prime numbers within a specific range and validates results.

Power_Numbers
Calculates power values (base^exponent).

Power_NUmbers_Test
Test cases for power number calculations.

Pİ_Calculator
Calculates the value of Pi using mathematical approximation methods.

Simple_Pİ_Calculator
A simplified Pi calculation program.

Pİ_Calculator_Test
Tests for Pi calculation programs.

🎮 Guessing & Logic Games

GuessTheNumber_Algorithm
Implements the algorithm logic for a number guessing game.

GuessTheNumber_User
User-interactive version of the guessing game.

👨‍💼 Object-Oriented & Data Programs

Employee_Program1
Basic employee management program using object-oriented concepts.

Employee_Program2
Extended version with additional employee attributes and logic.

Employee_Test
Test cases for employee-related programs.

Student_Program
Student information management using classes and structured data.

🔺 Geometry Programs

Triangle_Check
Checks whether given sides can form a valid triangle.

Triangle_2
Additional triangle-related calculations.

Triangle_Check_Test
Test cases for triangle validation logic.

📊 Data & Functional Programming

Frequency_Program
Calculates frequency of elements in a dataset.

Lambda_Functions_Program
Demonstrates the use of lambda expressions and functional programming concepts.

Ling_Features_Program_Students
Explores language features using student-based examples.

🌐 Computer Networks

Computer_Network
Fundamental computer network concepts and exercises.

Computer_Network_Test
Test cases related to network concepts.

Computer_Network_simulation_Final_Project
Final project involving computer network simulation and analysis.

💻 General Programming Practice

Computer_Program1 – Computer_Program7
A series of programming exercises covering basic syntax, control structures, and algorithmic thinking.
